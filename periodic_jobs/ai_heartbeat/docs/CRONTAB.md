...
0 8 * * * cd /opt/processes/mc_platform/context-infrastructure && .venv/bin/python periodic_jobs/ai_heartbeat/src/v0/jobs/ai_news_survey.py --mode daily >> /tmp/ai_news_survey.log 2>&1

# AI News Survey 周报 — 每周一 8:00 AM（发布到 Kit 订阅者）
0 8 * * 1 cd /opt/processes/mc_platform/context-infrastructure && .venv/bin/python periodic_jobs/ai_heartbeat/src/v0/jobs/ai_news_survey.py --mode weekly --publish-to-kit >> /tmp/ai_news_weekly.log 2>&1
```
